<template>
  <div class="bg-white rounded-xl shadow-sm p-5">
    <div class="flex items-center justify-between mb-4">
      <h2 class="text-lg font-medium">入库任务</h2>
    ...
    <el-dialog v-model="dialogVisible" title="新增入库任务" width="600px">
...
import { getInboundOrders, createInboundOrder, executeInboundOrder } from '@/api/order'
...
const loadData = async () => {
  loading.value = true
  try {
    const res = await getInboundOrders({ page: pagination.page, page_size: 20 })
...
const showDialog = () => {
  Object.assign(formData, { warehouse_id: null, items: [{}], remark: '' })
...
const handleExecute = async (row) => {
  try {
    await ElMessageBox.confirm('确定执行该入库任务？', '提示', { type: 'warning' })
